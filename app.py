...
st.set_page_config(
        ...
        menu_items={
            'Get Help': 'https://www.woobblr.co.in',
...
if authentication_status:
    ...
    func = ["Overview", "See the start and end values", "Drop Columns", "Drop Categorical Rows", "Drop Numeric Rows", "Rename Columns", "Display Plot graph", "Handle missing data", "Merge Data", 'Target Analysis', 
                    'Distribution of Columns which has numbers', 'Count Plots of Categorical Columns', 
                    'Box Plots', 'analyse the endpoints', 'Classify and analyse the data with rows and columns']
    excel_type =["vnd.ms-excel","vnd.openxmlformats-officedocument.spreadsheetml.sheet", "vnd.oasis.opendocument.spreadsheet", "vnd.oasis.opendocument.text"]
    authenticator.logout("Logout","sidebar")
    st.sidebar.title("Welcome {names}")
    ...
    if uploaded_file is not None:
        ...
        multi_function_selector = st.sidebar.multiselect("Select the operation you want to perform on the data you uploaded: ",func, default=["Overview"])
    ...
    # ==================================================================================================
        if "See the start and end values" in multi_function_selector:

            outliers_selection = st.multiselect("Enter or select Name of the columns to see the start and end values:", num_category)
    ...
    # ===================================================================================================================
        ...
        if "Display Plot graph" in multi_function_selector:
    ...
    # ==========================================================================================================================================
        ...
        if "Merge Data" in multi_function_selector:
        ...
        if 'Target Analysis' in multi_function_selector:
        ...
        if 'Distribution of Columns which has numbers' in multi_function_selector:
            ...
            if len(num_columns) == 0:
                ...
                st.subheader('Distribution of columns which has numbers')
                i = 0
        ...
        if 'Count Plots of Categorical Columns' in multi_function_selector:
        ...
        if 'Box Plots' in multi_function_selector:
            if len(num_columns) == 0:
                st.write('There is no numerical columns in the data.')
            else:
        ...
        if 'analyse the endpoints' in multi_function_selector:
            st.subheader('analyse the endpoints')
            c1, c2, c3 = st.columns([1, 2, 1])
            c2.dataframe(functions.number_of_outliers(data))

        if 'Classify and analyse the data with rows and columns' in multi_function_selector:
            ...
            if len(normal_cardi_columns) == 0:
                ...
                model_type = st.radio('Select Problem Type:', ('Relation between various categories', 'Classification'), key = 'model_type')
                ...
                if 'Target Analysis' not in multi_function_selector:   
                ...
                while (i < len(selected_cat_cols)):
                    ...
                    if model_type == 'Relation between various rows':
                        fig = px.box(data_1, y = target_column, color = selected_cat_cols[i])
                    else:
                        fig = px.histogram(data_1, color = selected_cat_cols[i], x = target_column)
                ...
                if high_cardi_columns:
                    if len(high_cardi_columns) == 1:
                        st.subheader('The following column has high unique values, that is why its boxplot was not plotted:')
                    else:
                        st.subheader('The following columns have high unique values, that is why its boxplot was not plotted:')
                    ...
                    if answer == 'Yes':
                        for i in high_cardi_columns:
                            fig = px.box(data_1, y = target_column, color = i)
                            st.plotly_chart(fig, use_container_width = True)
